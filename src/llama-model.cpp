...
void llama_model_base::create_tensor_qkv(llama_layer & layer, int bid,
    ...
    if (layer.wqkv) {
        ...
        if (!layer.wqkv_b) {
            // fused weights but separate biases (e.g., from --fuse-qkv conversion)
            layer.wq_b = create_tensor(tn(LLM_TENSOR_ATTN_Q, "bias", bid), {n_embd_q_}, TENSOR_NOT_REQUIRED | TENSOR_SKIP_IF_VIRTUAL);
            layer.wk_b = create_tensor(tn(LLM_TENSOR_ATTN_K, "bias", bid), {n_embd_k_}, TENSOR_NOT_REQUIRED | TENSOR_SKIP_IF_VIRTUAL);
            layer.wv_b = create_tensor(tn(LLM_TENSOR_ATTN_V, "bias", bid), {n_embd_v_}, TENSOR_NOT_REQUIRED | TENSOR_SKIP_IF_VIRTUAL);
        }
    } else {
